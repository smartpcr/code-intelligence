...
func (w *Writer) InsertEdge(ctx context.Context, in EdgeInput) (rec EdgeRecord, err error) {
	...
	rec, err = w.insertEdgeImpl(ctx, "InsertEdge", in, &fields)
	return rec, err
}
...
// through the shared `insertEdgeImpl` helper, and the
// "InsertObservedCallsEdge" op name is threaded through so that
// any SQLSTATE 42501 surfaces as a WriteContractViolation whose
// Op field names the real public entry point (not "InsertEdge")
// — operators triaging role-grant failures see the caller they
// actually invoked.
func (w *Writer) InsertObservedCallsEdge(ctx context.Context, in EdgeInput) (rec EdgeRecord, err error) {
	...
	rec, err = w.insertEdgeImpl(ctx, "InsertObservedCallsEdge", in, &fields)
	return rec, err
}
...
// "InsertObservedCallsEdge"). It is threaded through to
// `runInTx` → `classifyErr` so a SQLSTATE 42501 violation
// surfaces as a `WriteContractViolation` whose `Op` field names
// the actual entry point the caller invoked. It is also used in
// the wrapped error-context strings so operators triaging
// failures see consistent labelling between the audit log and
// any error chain they read.
//
