...
CLOSED. The `Workstream slug` column carries the exact operator-
confirmation slug the operator pinned in `.forge/memory/workstream-context.md`'s
"Open questions and operator answers" block; the slug appears verbatim
here so a `grep -F` from either side resolves to the same answer:

| Decision id              | Workstream slug (operator-pinned)                                  | Question                                                                                                                                                                  | Committed answer (binding for this story)                                                                                                                                                                                                                                          |
| ---                      | ---                                                                | ---                                                                                                                                                                       | ---                                                                                                                                                                                                                                                                                |
| `phase1-compose-path`    | `confirm-create-new-standalone-postgres-only-as-doc-shows`         | Should Phase 1 reuse `services/agent-memory/deploy/local/docker-compose.yml`, or create a new `tests/e2e/{phase-slug}/docker-compose.yml`, or symlink to the service-local file? | CREATE NEW. Phase 1 owns a standalone postgres-only compose file at `tests/e2e/phase-1-shared-additive-surfaces/docker-compose.yml` whose `postgres` service uses `build: { context: ../../../services/agent-memory/deploy/local/postgres, dockerfile: Dockerfile }` and `image: agent-memory/postgres:16-partman` -- i.e., the SAME `FROM postgres:16` Debian image plus `pg_partman` that the service-local stack produces (the upstream image, build context, and produced tag are defined at `services/agent-memory/deploy/local/postgres/Dockerfile` and `services/agent-memory/deploy/local/docker-compose.yml` lines 22-27). It mounts the init SQL from `services/agent-memory/deploy/local/postgres/init/` to `/docker-entrypoint-initdb.d`, but does NOT `include:` the full service-local compose (avoids dragging in qdrant / otel-collector for a phase that only needs postgres). Recorded in 0.4 and in Phase 1 Setup. |
...
- **Local**: This is a two-step workflow because the compose file lives at the repo root but the Go module root is `services/agent-memory` (the file `services/agent-memory/go.mod` is the only module manifest under `services/agent-memory/`; there is no root `go.work`). Step 1 -- from the repo root, run `docker compose -f tests/e2e/phase-1-shared-additive-surfaces/docker-compose.yml up -d` to start postgres. Step 2 -- export `AGENT_MEMORY_PG_URL` per the env-var convention used by the agent-memory test suite (the value is supplied by the compose file's documented mapping; tests read the env var, never a literal DSN), then `cd services/agent-memory` and run `go test ./internal/repoindexer/ast -count=1 -run 'TestDispatcher|TestLangMeta|TestErrParserUnavailable|TestMergeLangMeta|TestNormalizeHints|TestPass2bMultimap|TestPass2dOverrides'` for the dispatcher unit-test slice and `go test ./migrations -count=1 -run 'TestMigrator_Up_AppliesAll|TestMigrations_0022_EdgeKindOverrides'` for the migration apply / probe test. Both `go test` targets resolve relative to the `services/agent-memory` module root; running them from the repo root yields `no Go files in ...` and is incorrect.
...
```gherkin
@regression @no-pwsh
Scenario Outline: PowerShell extensions route to the parser under both build tags (routing is registration-only, NOT runtime-gated)
  Given the dispatcher constructed via defaultParsers() under <cgo>
  And the host has NO pwsh on PATH (exec.LookPath("pwsh") returns ErrNotFound)
  When selectParser("<file>", nil) is invoked
  Then the returned parser's Language() equals "powershell"
  And selectParser does NOT consult pwshBin or runtime availability (routing matches the implementation-plan.md Stage 6.2 contract that .ps1 / .psm1 / .psd1 route to the PowerShell parser under both CGO=on and CGO=off, independent of pwsh presence; the absent-pwsh sentinel is exercised separately by the @no-pwsh dispatcher-skip scenario above)
...
- **Local**: This is a two-step workflow because the compose file lives at the repo root but the Go module root is `services/agent-memory` (the file `services/agent-memory/go.mod` is the only module manifest under `services/agent-memory/`; there is no root `go.work`). Step 1 -- from the repo root, run `docker compose -f tests/e2e/phase-7-cross-cutting-validation/docker-compose.yml up -d` to bring up the three services. Export `AGENT_MEMORY_PG_URL`, `AGENT_MEMORY_QDRANT_URL`, `AGENT_MEMORY_OTEL_ENDPOINT` via the env-var convention already used by `.github/workflows/agent-memory-ci.yml` (the workflow keys these in its job `env`; local developers can `source` a `.envrc` that mirrors the same keys, never inlining the literal DSNs in the doc or in shell history). Step 2 -- `cd services/agent-memory` and run, in order: `CGO_ENABLED=1 go test ./internal/repoindexer/ast -count=1` (targeted), `CGO_ENABLED=0 go test ./internal/repoindexer/ast -count=1` (CGO-off portable matrix), `CGO_ENABLED=1 go test ./... -count=1` (full service suite), and `make lint` (the `Makefile` lives at `services/agent-memory/Makefile`). All four commands resolve relative to the `services/agent-memory` module root; running them from the repo root produces module-resolution errors and is incorrect.
