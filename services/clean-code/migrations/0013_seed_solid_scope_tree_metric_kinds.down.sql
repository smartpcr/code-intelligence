...
-- Deletes the three SOLID-pack scope-tree catalog rows
-- seeded by the UP half, scoped to the EXACT
-- `(metric_kind, metric_version)` tuples at v=1. A
-- Steward-curated row at a DIFFERENT `metric_version`
-- (>=2) is preserved by the tuple predicate.
--
-- Caveat: any `clean_code.metric_sample` rows that reference
-- these `(metric_kind, metric_version)` tuples will block
-- the DELETE via the FK `metric_sample_metric_kind_fk` at
