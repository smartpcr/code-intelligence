...
// ilikePatternEscaper escapes the three PostgreSQL `ILIKE`
// pattern meta-characters so an operator-supplied substring
// is treated as a LITERAL match, not as a pattern. We escape
// the default ESCAPE character (`\`) FIRST so the `\` we
// inject when escaping `%` and `_` is not itself re-escaped.
//
// Order in NewReplacer does not matter for correctness here:
// `NewReplacer` matches against the ORIGINAL input string at
// each position and never re-processes the replacement text,
// so a `\` we inject as part of escaping `%` or `_` is not
// itself re-escaped.
var ilikePatternEscaper = strings.NewReplacer(`\`, `\\`, `%`, `\%`, `_`, `\_`)

// escapeILIKEPattern returns `s` with PostgreSQL `ILIKE`
// pattern meta-characters escaped (see [ilikePatternEscaper]).
// Use this on any operator-supplied substring BEFORE wrapping
// it with surrounding `%` wildcards.
...
func (h *Handler) handleReadRepos(w http.ResponseWriter, r *http.Request) {
	...
	// `_` and `%` characters in the input would silently act
	// as PostgreSQL ILIKE wildcards (e.g. `my_repo` would also
	// match `myXrepo`), which is surprising for an operator
	// who expects substring-match semantics.
