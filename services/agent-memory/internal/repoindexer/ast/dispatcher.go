...
//
// The canonical type is `*slog.Logger` ╬ô├ç├╢ the dispatcher emits
...
// dispatcherParsersForTest exposes the dispatcher's extension╬ô├Ñ├åparser
...
// given file to, applying the architecture ┬º4.1 contract:
//
...
func (d *Dispatcher) selectParser(relPath string, eventHints []string) Parser {
	...
	// per-event slice is empty (architecture ┬º4.1, tested by
...
//     `use std::fmt::Display` ╬ô├º├å `["Display"]`).
...
//   - Pass 2b (static_calls):  AMBIGUITY-AWARE ╬ô├ç├╢ a bare call
...
func (d *Dispatcher) EmitFile(ctx context.Context, ev repoindexer.EmitFileEvent) (repoindexer.EmitResult, error) {
	...
	// Pass 0: imports ╬ô├Ñ├å package nodes + imports edges
	...
	// Pass 1a: classes (build local-class set + sig╬ô├Ñ├åNodeID map).
	// Emits one `class` node per ClassDecl PLUS one `contains`
	// edge `file ╬ô├Ñ├å class` so the file╬ô├Ñ├åclass╬ô├Ñ├åmethod╬ô├Ñ├åblock
	...
	for _, m := range result.Methods {
		...
		// Go's pointer-receiver `*Foo.Bar` ╬ô├Ñ├å `Foo.Bar` alias).
		...
		// Emit the parent╬ô├Ñ├åmethod `contains` edge using the
		...
		// Inline publish for the Method node ╬ô├ç├╢ fires AFTER
		// the parent╬ô├Ñ├åmethod contains edge so the publisher
		...
		// body exceeds the Γö¼┬║8.2 logical-line threshold the
		// dispatcher inserts two Block nodes (entry, exit)
		// as children of the method and emits a method╬ô├Ñ├åblock
	...
	// Pass 2b: static_calls ╬ô├ç├╢ ambiguity-aware bare-name + scoped
	...
	for _, m := range result.Methods {
		...
		if m.EnclosingClass != "" {
			for _, callee := range m.ReceiverCalls {
				...
				// ones). A single-node set ╬ô├ç├╢ even one populated by
				// both a primary key and a ReceiverAlias ╬ô├ç├╢ still
	...
	// accesses (architecture Γö¼┬║5.3 ╬ô├ç├╢ "every recognised member
	...
	// Pass 2d: overrides (impl method ╬ô├Ñ├å trait-default trait
	...
	// parent╬ô├Ñ├ånode `contains` edge per the canonical contract
	// (`TestDispatcher_EmbeddingPublisher_PublishesAfterContainsEdge`).
...
// on an `imports` Edge ╬ô├ç├╢ `symbols`, `alias`, `is_type_only`, `line`.
// The `symbols` key is what Stage 5.3 requires on the Rust imports
// edge (e.g. `use std::fmt::Display` ╬ô├º├å `["Display"]`).
...
// qualified name (e.g. "Foo.bar" ╬ô├Ñ├å "bar", "free_fn" ╬ô├Ñ├å "free_fn").
...
// Γö¼┬║4.4.2 "first-class key wins on collision" rule. Presence ╬ô├ç├╢
// not value ╬ô├ç├╢ of a key in `out` is the gate: a first-class attr
...
// first-class-key-wins rule (Γö¼┬║4.4.2).
