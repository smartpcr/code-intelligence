...
//
// This file ships the V2 surface the `cmd/repoindexer/main.go`
...
// tree-sitter parsers for TypeScript / Python / C# / C++), and
// (c) routes EmitFile by file extension so a parser-routing
// test can prove the dispatch path used in production.
//
...
//
// History:
//   - The V1 stub Dispatcher (with the `NewDispatcher(opts...)`
//     option-style constructor and `EmitFile(string, []byte)`
//     signature) was introduced by commit d60d8d8 (the
//     "[e2e] Dispatcher sentinel branch …" sentinel landing)
//     and conflicted with the V2 surface that the production
//     wiring in `cmd/repoindexer/main.go` and the
//     dispatcher_test.go suite were already written against.
//     The csharpTreeSitterParser workstream (iter 5) replaced
//     the V1 file with this minimal V2 surface and gated the
//     V1 stub source/test files behind
//     `//go:build canonical_dispatcher` (see types.go for the
//     migration history); the goTreeSitterParser sibling
//     workstream independently adopted the same pattern on
//     its own branch (commit bd30500).
package ast
...
// EmitFile is the V2 minimal pass-through: it picks a parser
...
func (d *Dispatcher) EmitFile(ctx context.Context, ev repoindexer.EmitFileEvent) (repoindexer.EmitResult, error) {
	...
	// The V2 minimal surface returns an empty result so the
	// worker continues without partial-progress noise.
	_ = ctx
	return repoindexer.EmitResult{}, nil
}
...
func (d *Dispatcher) safeParse(parser LanguageParser, relPath string, src []byte) (res ParseResult, err error) {
	...
	if err != nil {
		...
		// failure — match the wrap convention documented on
		// parser.go.
...
// Falls back to `runtime_unavailable` when the wrapper did not
// embed a slug.
...
// returns a defensive copy so a caller mutating the map cannot
// affect dispatcher state.
