...
`MethodDecl` / `Import` envelope. The table below is the
authoritative mapping; per-construct extraction rules are
the tech spec's responsibility.
...
| **Go** | The receiver type name (stripped of pointer marker `*`) when the method has a receiver clause `func (r *Foo) Bar(...)`. Free functions have empty receiver. | text between the outer `(` `)`. | The receiver identifier's qualified calls: when the receiver is `r`, every `r.X(` call is a ReceiverCall. The parser tracks the receiver-binding name per method to recognise the right prefix. | empty in v1 (Go has no method modifiers; `attrs_json["receiver"]` captures the bound name + pointer flag). |
...
### 4.4 attrs_json extensions

The dispatcher's existing attrs writers (`classAttrs`,
`methodAttrs`, `externalPackageAttrs`, `importEdgeAttrs`,
`memberEdgeAttrs`) ALREADY persist `language`, `decl_kind`,
`modifiers`, `params_raw`, etc. (`dispatcher.go` lines
832-961). This story adds the following keys without changing
the attrs writers themselves -- the new parsers populate the
corresponding `ClassDecl.Kind` / `MethodDecl.Modifiers`
slots, and the existing writers serialise them.

| New `attrs_json` key | Carried on | Populated by | Purpose |
| --- | --- | --- | --- |
| `decl_kind` values: `struct`, `union`, `enum`, `record`, `trait`, `type_alias` | class node | Section 4.1 mapping | Lets consumers filter without re-parsing the canonical signature. |
| `receiver` (string) | method node | Go parser | Records the receiver-name (`r`) the method bound; used by the parser internally and persisted for downstream consumers. |
| `receiver_ptr` (bool) | method node | Go parser | Distinguishes `func (r Foo)` from `func (r *Foo)`. |
| `trait` (string) | method node | Rust parser | When the method came from an `impl Trait for Type`, names the trait. |
| `namespace` (string) | class node | C++ / C# parsers | Records the enclosing namespace path; the canonical signature drops it (the `.` flattening rule in Section 4.1). |
| `base_access` (map[name]string) | class node | C++ parser | Records the access specifier (`public`, `protected`, `private`) for each base class. |
| `dot_import` / `blank_import` / `is_static` (bool) | imports edge | Go / C# parsers | Distinguishes import-style variants. |
| `cmdlet_verb` (string), `module_kind` (`Import-Module` / `using_module` / `dot_source`) | imports edge | PowerShell parser | Distinguishes the four PowerShell module-acquisition forms. |

No schema migration is required -- `attrs_json` is `jsonb`
end-to-end (see Stage 2.1 / `migration 0001`).
...
Note on `.h`: C and C++ both claim header files in practice. The
extension routing prefers a single parser per extension (the
existing `buildExtMap` is a 1:1 map). v1 routes `.h` to the C
parser; `.hpp` / `.hh` / `.hxx` route to C++. Repos with C++
projects that use `.h` for headers can override via
`repo.language_hints[]` populated with `cpp` (the dispatcher's
hint precedence falls through to the language-id lookup, which
selects the C++ parser when the extension does not match the
hint -- per the existing `selectParser` resolution order). The
caveat is documented in the open-question block at the end of
this file because the operator may prefer the inverse default.

### 5.3 Dispatcher hooks (UNCHANGED)

The dispatcher's `selectParser` /
`safeParse` / `emit` / `emitImportsEdges` entry points are
unchanged. The only edit is `defaultParsers()` and the
`normalizeHints` alias table (Section 3).
...
Ship **Option A** (scanner-only) and surface an Open Question
(see end of this file) so the operator can promote to Option B
in a follow-up story if PowerShell coverage matters more than
build-matrix simplicity. The scanner approach is the lowest-risk
landing for the 13-point budget; trying to wire a new C
...
- The `.h` vs `.hpp` rule is exercised by a routing test that
  ...
  hint-override case asserting that
  `selectParser("a.h", []string{"cpp"})` still returns the C
  parser (extension is highest precedence).
...
| **R2** -- Go method receiver pointer / value flips. `func (r *Foo) Bar()` and `func (r Foo) Bar()` are distinct signatures but normalise to the same QualifiedName. | `attrs_json["receiver_ptr"]` captures the distinction; the canonical signature includes the parameter list verbatim, so `(g *Greeter)` vs `(g Greeter)` produce distinct fingerprints anyway because `ParamSignature` is empty (Go puts the receiver in its own clause). Open question for the operator: do we want a synthetic `&` prefix on the QualifiedName to keep value/pointer methods distinct at the fingerprint level? (See open-questions block.) |
| **R3** -- C# `partial class` declarations across multiple files. v1 treats each partial file as its own ClassDecl with the same QualifiedName but different relPath -> different canonical signatures, hence distinct nodes. The cross-file resolver will stitch them via a future `partial_of` edge. | Documented as a future story; no v1 work required. The `attrs_json["partial"]=true` flag is set on each partial class so the consumer can group by name. |
| **R4** -- Rust trait method default impl. A trait declaration with a default-bodied method produces both a MethodDecl on the trait class (Kind="trait") AND, separately, an `impl Trait for Type` method shadows it on the Type class. | Both are emitted -- the resolver's "drop on ambiguity" rule (`buildCalleeIndex` collisions) handles the bare-name resolution correctly. Open question on whether to emit an `overrides` edge from Type.method -> Trait.method in v1 (per A4 we drop it; documented). |
...
| **R6** -- `.h` ambiguity. C / C++ both use `.h`; the v1 routing sends `.h` to the C parser. | Documented in Section 5.2; operators with C++ `.h` projects override via `repo.language_hints[]`. The dispatcher's hint precedence cannot override extension routing today (extension is higher) -- open question on whether to invert the precedence when the hint explicitly names `cpp`. |

## 10. Open Items and Out-of-Story Workstreams

Items the operator may want to pin OR that defer to a later
story:

- **`.h` routing default** -- v1 routes to C; the operator may
  prefer C++. See open-questions block.
- **PowerShell grammar acquisition** -- Option A (scanner)
  ships in v1; promoting to Option B (smacker-style binding
  for an upstream `tree-sitter-powershell` fork) is a
  follow-up. See open-questions block.
- **Cross-file resolver** -- still deferred per A4 / A5.
  ...
  carry verbatim names on `attrs_json` so the future
  resolver can re-emit them.
...
- **C++ namespace + template handling beyond name extraction**
  ...
  identity. `attrs_json["template_params"]` is recorded
  but not used for resolution.

## Iteration Summary

- Path: `docs/stories/code-intelligence-AST-PARSER-FOR-ADDIT/architecture.md`
- Covers from the story description: (1) v1 extraction scope per
  language (Section 4 matrices), (2) parser library coverage
  for C / C++ / C# / Go / Rust and the PowerShell strategy
  decision (Section 6), (3) new parser file plan (Section 2.1
  table), (4) extension registration in `parsers_cgo.go` /
  `parsers_nocgo.go` (Section 3), (5) mapping of language
  constructs into existing graph model
  (Sections 4.1 - 4.4), (6) note on fixture-driven tests
  (Section 8.4 references the implementation plan / tech spec
  for the file list and fixture content), (7) cross-language
  ...
  language scenarios (Section 7.1 / 7.2 / 7.3) and the failure
  case (Section 7.4).
...
### Iteration 1 open questions for the operator

- The PowerShell grammar decision (Section 6).
- The `.h` extension routing default (Section 5.2).
- The Go value-vs-pointer receiver fingerprint disambiguation
  rule (Section 9 R2).
- Whether v1 should emit a synthetic `overrides` edge for
  Rust trait-impl method shadowing (Section 9 R4).

If `tech-spec.md` or `implementation-plan.md` later picks a
different default for any of these, this file should be
updated in the same iteration to keep the three docs aligned.
