...
func (s *scenarioState) aFindingsJSONPreviouslyWrittenByAnAnalyzeRun() error {
	// Golden snapshot approach: read the committed findings.json
	// that represents a prior analyze run's output. The golden
	// fixture was generated by rendering minimalArtifact() through
	// report.JSON{}.Render() and committed as testdata — this is
	// "a findings.json previously written by an analyze run."
	goldenFindings := filepath.Join(testdataDir(), "report_rerender", "findings.json")
	data, err := os.ReadFile(goldenFindings)
	if err != nil {
		return fmt.Errorf("read golden findings.json: %w", err)
	}

	// Copy to temp dir for the binary to consume.
	s.findingsPath = filepath.Join(s.tmpDir, "findings.json")
	if err := os.WriteFile(s.findingsPath, data, 0o644); err != nil {
		return err
	}

	// The golden expected.md represents "the markdown that the
	// analyze run emitted" — committed alongside findings.json.
	s.expectedMDPath = filepath.Join(testdataDir(), "report_rerender", "expected.md")
	return nil
}
...
func (s *scenarioState) goTestTagsProdDevpolicyRuns() error {
	// Run only the LoadUnsignedBundle prod test — this is the
	// test that calls devpolicy.LoadUnsignedBundle(...) and
	// asserts the returned error equals ErrDevModeUnavailable.
	cmd := exec.Command("go", "test", "-tags", "prod", "-count=1", "-v",
		"-run", "TestLoadUnsignedBundle", "./internal/cli/devpolicy/...")
	cmd.Dir = s.modRoot
	var stdout, stderr bytes.Buffer
	cmd.Stdout = &stdout
	cmd.Stderr = &stderr
...
func (s *scenarioState) replayMDIsByteIdenticalToTheMarkdownThatTheAnalyzeRunEmitted() error {
	...
	// Read the golden expected.md (committed fixture representing
	// the markdown the analyze run emitted).
	expectedMD, err := os.ReadFile(s.expectedMDPath)
	if err != nil {
		return fmt.Errorf("read golden expected.md: %w", err)
	}

	// Read the replay.md produced by cleanc report --out.
	replayPath := filepath.Join(s.tmpDir, "replay.md")
	...
	if !bytes.Equal(got, expectedMD) {
		return fmt.Errorf("replay.md is not byte-identical to golden expected.md\n--- expected (%d bytes) ---\n%s\n--- got (%d bytes) ---\n%s",
			len(expectedMD), string(expectedMD), len(got), string(got))
	}
	return nil
}
...
func (s *scenarioState) theProdGatedTestPassesAndAssertsErrDevModeUnavailableWithMessage(msg string) error {
	// 1. Verify the subprocess prod test passes — this runs
	//    TestLoadUnsignedBundle_ProdReturnsErrDevModeUnavailable
	//    which calls devpolicy.LoadUnsignedBundle(...) and asserts
	//    error identity (==) and exact message match.
	...
	// Verify the specific LoadUnsignedBundle test ran.
	if !strings.Contains(combined, "TestLoadUnsignedBundle_ProdReturnsErrDevModeUnavailable") {
		return fmt.Errorf("go test output does not contain TestLoadUnsignedBundle_ProdReturnsErrDevModeUnavailable\noutput:\n%s", combined)
	}

	// 2. Call devpolicy.LoadUnsignedBundle in-process to verify
	//    the symbol is callable. In this dev build the error is
	//    ErrLoaderNotYetImplemented; the prod proof is above.
	_, err := devpolicy.LoadUnsignedBundle(context.Background(), devpolicy.LoaderSource{UseEmbedded: true})
	if err == nil {
		return fmt.Errorf("devpolicy.LoadUnsignedBundle returned nil error; want a non-nil sentinel")
	}

	// 3. Verify the ErrDevModeUnavailable error message is an
	//    exact match (not substring) against the scenario-pinned text.
	const wantMsg = "devpolicy: dev-mode policy bypass not available in prod build"
	if got := devpolicy.ErrDevModeUnavailable.Error(); got != wantMsg {
		return fmt.Errorf("ErrDevModeUnavailable.Error() = %q; want exact %q", got, wantMsg)
	}
	// Verify the scenario-supplied message is contained in the
	// sentinel (the scenario text omits the "devpolicy: " prefix).
	if !strings.Contains(wantMsg, msg) {
		return fmt.Errorf("sentinel message %q does not contain scenario-pinned %q", wantMsg, msg)
	}
	return nil
}
