...
| `head_sha`                  | `string`           | `git HEAD` SHA the analysis ran against, detected by `repocontext.DetectHeadSHA`. The literal string `"working-copy"` (the `repocontext.HeadSHAWorkingCopySentinel` constant) when the repo is not a git checkout or has no commits; `DetectHeadSHA` never returns an empty string (see `repocontext/doc.go`), so consumers MUST compare against the sentinel rather than `""` to detect non-git inputs. |
| `policy_version_id`         | `string` (UUID)    | UUID of the `steward.PolicyVersion` the engine evaluated against. **Deterministic**: synthesised as a UUID-v5 over the loaded rule-id set, so re-running the dev loader against the same rule packs yields a byte-identical id (tech-spec constraint C11, architecture Sec 5.8; pinned by `internal/cli/devpolicy/loader.go` `Bundle.PolicyVersion` doc and `internal/cli/devpolicy/unsigned_dev.go` `Load` "Determinism" note). Downstream cache keys and reproducibility checks can rely on equality across runs over the same packs. |
