...
The story `code-intelligence:AST-PARSER-FOR-ADDIT` is decomposed into one stage workstream per language (or language group). This stage owns *only* the Go tree-sitter parser; the C/C++/C#/Rust/PowerShell tree-sitter parser files (`parser_treesitter_c.go`, `parser_treesitter_cpp.go`, `parser_treesitter_csharp.go`, `parser_treesitter_rust.go`, `parser_powershell.go`, and their `_test.go` siblings) belong to other active worktrees on this same story and will land via their own stage merges:

| Sibling stage worktree                                          | Branch slug                                                                          | Parser files owned                                                                                |
| --------------------------------------------------------------- | ------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------- |
| `stage-3.1-ctreesitterparser-implementation`                    | `phase-c-and-cpp-parsers-stage-ctreesitterparser-implementation`                     | `parser_treesitter_c.go`, `parser_treesitter_c_test.go` (and the C++ sibling on the same branch)  |
...
Per-stage worktrees are visible locally via `git worktree list`. Any evaluator review of this Go-parser stage should treat the absence of C/C++/C#/Rust/PS parser files as **expected**, not as missing deliverables — `parsers_cgo.go` registers `NewTreeSitterTypeScriptParser` / `NewTreeSitterPythonParser` / `NewTreeSitterGoParser` on this branch precisely because the other registrations are landing on the sibling stage branches above and will be merged onto `feature/memory` separately. The file-list comment block in `services\agent-memory\internal\repoindexer\ast\parsers_cgo.go` documents the same scope split inline next to `defaultParsers()`.
