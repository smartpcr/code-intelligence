...
> - **eval.gate synchronous SOLID delegation:** `eval.gate` DELEGATES the rule pass to the SOLID Rule Engine's synchronous mode `rule_engine.RunSync(ctx, repo_id, sha, scope?, policy_version_id)` which writes **ONE `evaluation_run(caller='eval_gate')` + ONE `evaluation_verdict` + N `finding` rows in the same transaction** and returns all three IDs; the verdict is computed by the engine via severity-rollup over the freshly-written findings, and eval.gate does NOT separately append `evaluation_verdict` on the happy path. eval.gate writes `evaluation_verdict` directly ONLY on the two short-circuit degraded paths (signature-invalid, samples_pending) where the engine is NOT invoked. Per architecture Sec 3.6 lines 526-540 and Sec 4.2 lines 755-762.
...
### Implementation Steps
- [ ] Implement `eval.gate(repo_id, sha, scope?)` verb per architecture Sec 3.7 lines 548-570 and Sec 4.2 lines 755-764. Sequence: (1) resolve active `policy_version_id` via latest `policy_activation` row; (2) verify `policy_version.signature` against the Steward signing key -- on mismatch return `verdict='warn', degraded=true, degraded_reason='policy_signature_invalid'` to the caller WITHOUT delegating to the Rule Engine and WITHOUT writing findings (the Rule Engine never runs on an invalid policy signature; per architecture Sec 1.6 `policy-signing-required` pin, the gate never blocks); (3) check Phase 2 completion for the SHA -- if any required input is `samples_pending` return `verdict='warn', degraded=true, degraded_reason='samples_pending'` WITHOUT delegating and WITHOUT writing findings; (4) DELEGATE to the SOLID Rule Engine **synchronous mode** (Stage 5.7) `rule_engine.RunSync(ctx, repo_id, sha, scope?, policy_version_id) -> (evaluation_run_id, evaluation_verdict_id, []finding_id)` -- per architecture Sec 3.6 lines 526-540 and Sec 4.2 lines 760-762 the engine writes **ONE `evaluation_run` + ONE `evaluation_verdict` + N `finding` rows in the same transaction** and returns their IDs; (5) eval.gate **does NOT** write its own `evaluation_verdict` row -- the verdict that the engine wrote (severity-rollup: `block` if any unmuted finding has `severity='block'`; `warn` if any has `severity='warn'`; `pass` otherwise) is the canonical one and eval.gate simply reads it back to shape the response; (6) for the two short-circuit degraded paths (signature-invalid, samples_pending) where the Rule Engine was NOT invoked, eval.gate appends ONE `evaluation_verdict(verdict_id, evaluation_run_id=NULL, scope?, verdict='warn', degraded=true, degraded_reason, settled_at)` row via its `clean_code_evaluator` grant (Stage 1.5) so the verdict is auditable even though no run exists -- this is the only path on which eval.gate writes an `evaluation_verdict` directly.
...
- [ ] Add `internal/evaluator/gate_test.go` covering: clean pass path -- exactly one `evaluation_run` + one `evaluation_verdict` + N `finding` rows appear, all written by `RuleEngine.RunSync` in the same transaction with `caller='eval_gate'`; block on severity=block finding via the rollup; warn on degraded (policy signature invalid) -- NO Rule Engine call, zero findings, ONE `evaluation_verdict(evaluation_run_id=NULL, verdict='warn', degraded=true, degraded_reason='policy_signature_invalid')` written by the Evaluator directly; warn on degraded (samples_pending) -- NO Rule Engine call, zero findings, ONE Evaluator-written verdict; degraded_reason rejects `percentile_stale`.
...
- [ ] Scenario: degraded-maps-to-warn -- Given a samples_pending degraded condition (no findings written), When `eval.gate` runs, Then the Rule Engine is NOT invoked, zero new `finding` rows are appended, and ONE `evaluation_verdict(evaluation_run_id=NULL, verdict='warn', degraded=true, degraded_reason='samples_pending')` row IS written directly by the Evaluator (via the `clean_code_evaluator` grant from Stage 1.5) so the auditable verdict still exists.
- [ ] Scenario: gate-does-not-double-write-verdict -- Given the clean-pass path where the Rule Engine wrote one `evaluation_verdict`, When eval.gate returns, Then exactly ONE `evaluation_verdict` row exists for that run (eval.gate did NOT append a second one); on the signature-invalid / samples_pending paths the Rule Engine was not invoked and the single verdict row was written by eval.gate itself.
